...
for _, symbol_row in df_symbols.iterrows():
    ...
    ind_5_below_minus_5_count = 0  # Counter for consecutive ind_5 < -5
    ...
    for _, row in df_data.iterrows():
        ...
        if position_open:
            ...
            if pd.notna(ind_5):  # Handle NaN for safety
                if ind_5 < -5:
                    ind_5_below_minus_5_count += 1
                else:
                    ind_5_below_minus_5_count = 0  # Reset if ind_5 >= -5
            else:
                ind_5_below_minus_5_count = 0  # Reset if ind_5 is NaN

            # Check if sell triggered: ind_5 < -7 OR ind_5 < -5 for two consecutive rows
            if pd.notna(ind_5) and (ind_5 < -7 or ind_5_below_minus_5_count >= 3):
                ...
                print(f"Sold due to {'ind_5 < -7' if ind_5 < -7 else 'ind_5 < -5 for two consecutive rows'}: zysk={zysk:.2f} ({length} days)")
                position_open = False
                total_shares = 0
                total_invested_symbol = 0
                num_purchases = 0
                ...
                max_value = 0
                daily_states = []
                ind_5_below_minus_5_count = 0  # Reset counter after sell
                continue
        ...
        if ind_22 > 3 or ind_7 > 0:
            ...
            if not position_open:
                ...
                trailing_stop = 0.0
                trailing_active = False
                ind_5_below_minus_5_count = 0  # Reset counter when opening position
            else:
                num_purchases += 1
            current_value = total_shares * current_price
