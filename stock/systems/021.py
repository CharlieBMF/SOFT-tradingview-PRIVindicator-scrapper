...
for _, symbol_row in df_symbols.iterrows():
    ...
    # NOWE: Licznik małych zakupów dla tego symbolu
    small_buy_counter = 0
    MAX_SMALL_BUYS = 10  # Maksymalna liczba małych zakupów
    # Licznik kolejnych TickerRelative dla warunku -4 < ind_5 < 0
    ...
    for _, row in df_data.iterrows():
        ...
        if position_open:
            ...
            if ind_5 < -5:
                ...
                small_buy_counter = 0  # Reset licznika małych zakupów po sprzedaży
                continue
        ...
        if main_buy_trigger:
            ...
            if amount > 0:
                ...
                small_buy_counter = 0  # Reset licznika małych zakupów przy main_buy_trigger

        # Small buy - jeśli pozycja otwarta, NIE ma głównego triggera, jesteśmy stratni (zysk_strata < 0) i liczba małych zakupów < 5
        if position_open and not main_buy_trigger and zysk_strata < 0 and small_buy_counter < MAX_SMALL_BUYS:
            amount = 15.0
            buy_type = 'small'
            buy_price = current_price
            shares_bought = amount / buy_price
            ...
            total_invested_symbol += amount
            num_purchases += 1
            small_buy_counter += 1  # Zwiększ licznik małych zakupów
            current_value = (shares_from_22 + shares_from_7 + shares_from_small) * current_price
            max_value = max(max_value, current_value)
            zysk_strata = current_value - total_invested_symbol
            daily_states.append((tr, zysk_strata))
            print(f"Added ({buy_type}): amount={amount}, price={buy_price:.2f}, shares={shares_bought:.4f}, small_buy_counter={small_buy_counter}")
            print(f"Value={current_value:.2f}, invested={total_invested_symbol:.2f}, zysk={zysk_strata:.2f}")
