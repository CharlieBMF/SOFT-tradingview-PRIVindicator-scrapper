...
for _, symbol_row in df_symbols.iterrows():
    ...
    # Counter for consecutive TickerRelative for condition -4 < ind_5 < 0
    ...
    for _, row in df_data.iterrows():
        ...
        # total_shares includes all types of shares
        total_shares = shares_from_22 + shares_from_7 + shares_from_small
        zysk_strata = 0  # Initialize
        if position_open:
            ...
            if ind_5 < -5:
                # Sell all (including shares_from_7 and small)
                current_value = total_shares * current_price
                ...
                print(f"Sold ALL (incl. ind_7 and small) due to ind_5 < -5: zysk={zysk:.2f} ({length} days)")
                position_open = False
                shares_from_22 = 0
                invested_from_22 = 0
                shares_from_7 = 0
                ...
                max_value = 0
                daily_states = []
                consecutive_ind5_condition = 0  # Reset counter
                continue
            elif ind_5 < 0 and ind_5 > -4:
                # Count consecutive TickerRelative for condition -4 < ind_5 < 0
                consecutive_ind5_condition += 1
                if consecutive_ind5_condition >= 3:
                    # Sell shares_from_small only
                    if shares_from_small > 0:
                        ...
                        if shares_from_22 == 0 and shares_from_7 == 0:
                            ...
                            max_value = 0
                            daily_states = []
                        consecutive_ind5_condition = 0  # Reset counter after sale
                continue  # Skip buying after checking sell condition
            else:
                # Reset counter if condition -4 < ind_5 < 0 is not met
                consecutive_ind5_condition = 0
        ...
        if main_buy_trigger:
            ...
            if ind_22 == 6:
                ...
                main_buy_trigger = False  # If doesn't match amounts
            ...
            if amount > 0:
                ...
                zysk_strata = current_value - total_invested_symbol  # Update after purchase
                if position_open:
                    daily_states.append((tr, zysk_strata))
        ...
        # Small buy - if position open, no main buy trigger, and at a loss (zysk_strata < 0)
    ...
    if position_open:
        ...
        print(f"Open position at end (incl. ind_7 and small): value={current_value:.2f}, invested={total_invested_symbol:.2f}, zysk={zysk_strata:.2f}")
