...
for _, symbol_row in df_symbols.iterrows():
    ...
    consecutive_ind5_condition = 0  # For -4 < ind_5 < 0 condition
    consecutive_ind5_minus5 = 0  # NEW: For ind_5 < -5 condition
    ...
    for _, row in df_data.iterrows():
        ...
        if position_open:
            ...
            if ind_5 < -7 or (ind_5 < -5 and consecutive_ind5_minus5 >= 1):
                # Sell all (w tym small)
                current_value = total_shares * current_price
                zysk = current_value - total_invested_symbol
                length = tr - open_tr
                sold_type = 'all_ind5_lt_minus7' if ind_5 < -7 else 'all_ind5_lt_minus5_consecutive'
                positions.append({
                    'open_tr': open_tr,
                    'close_tr': tr,
                    'length': length,
                ...
                print(f"Sold ALL (incl. small) due to {sold_type}: zysk={zysk:.2f} ({length} days)")
                position_open = False
                shares_from_22 = 0
                invested_from_22 = 0
                shares_from_7 = 0
                ...
                consecutive_ind5_minus5 = 0  # Reset licznika
                continue
            elif ind_5 < -5:
                # Increment counter for ind_5 < -5
                consecutive_ind5_minus5 += 1
            else:
                # Reset counter if ind_5 >= -5
                consecutive_ind5_minus5 = 0

            # Check partial sell condition for -4 < ind_5 < 0
            if ind_5 < 0 and ind_5 > -4:
