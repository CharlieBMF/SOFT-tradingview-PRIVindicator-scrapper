...
for _, symbol_row in df_symbols.iterrows():
    ...
    # NOWE: Liczniki dla small buys
    small_buy_total_count = 0  # Kumulatywna liczba small buys w pozycji (dla rosnącej kwoty)
    small_buy_sequence_count = 0  # Sekwencyjny licznik (max 5, reset przy main_buy)
    ...
    for _, row in df_data.iterrows():
        ...
        if position_open:
            ...
            if ind_5 < -5:
                ...
                small_buy_total_count = 0  # Reset kumulatywnego licznika
                small_buy_sequence_count = 0  # Reset sekwencyjnego licznika
                continue
                ...
                if consecutive_ind5_condition >= 3:
                    ...
                    if shares_from_7 > 0:
                        ...
                        shares_from_7 = 0
                        invested_from_7 = 0
                        total_invested_symbol -= invested_from_7  # Poprawka: odejmij od total
                        ...
                        if shares_from_22 == 0 and shares_from_small == 0:
                            ...
                            max_value = 0
                            daily_states = []
                            small_buy_total_count = 0
                            small_buy_sequence_count = 0
                        consecutive_ind5_condition = 0  # Reset licznika po sprzedaży
                        small_buy_sequence_count = 0  # Reset sekwencyjnego (total zostaje)
                    ...
                    if shares_from_small > 0:
                        ...
                        shares_from_small = 0
                        invested_from_small = 0
                        total_invested_symbol -= invested_from_small  # Poprawka: odejmij od total
                        # Check if position still open
                        if shares_from_22 == 0 and shares_from_7 == 0:
                            ...
                            small_buy_sequence_count = 0
                        consecutive_ind5_condition = 0  # Reset licznika po sprzedaży
                        small_buy_sequence_count = 0  # Reset sekwencyjnego (total zostaje, ale skoro small sprzedane, można rozważyć reset total; tu zostawiamy dla kontynuacji pozycji)
        ...
        if main_buy_trigger:
            ...
            if amount > 0:
                ...
                if not position_open:
                    ...
                    trailing_stop = 0.0
                    trailing_active = False
                    # Inicjalizacja liczników small buy przy otwarciu pozycji
                    small_buy_total_count = 0
                    small_buy_sequence_count = 0
                else:
                    num_purchases += 1
                ...
                # Reset sekwencyjnego licznika przy main buy (total zostaje, kwota rośnie dalej)
                small_buy_sequence_count = 0

        # Small buy - jeśli pozycja otwarta, NIE ma głównego triggera i jesteśmy stratni (zysk_strata < 0)
        if position_open and not main_buy_trigger and zysk_strata < 0 and small_buy_sequence_count < 10:
            amount = 10.0 + small_buy_total_count  # Kwota rośnie na podstawie kumulatywnego licznika
            buy_type = 'small'
            buy_price = current_price
            shares_bought = amount / buy_price
            ...
            total_invested_symbol += amount
            num_purchases += 1
            small_buy_total_count += 1  # Inkrementuj kumulatywny (kwota dla następnego)
            small_buy_sequence_count += 1  # Inkrementuj sekwencyjny (max 5)
            current_value = (shares_from_22 + shares_from_7 + shares_from_small) * current_price
            max_value = max(max_value, current_value)
            zysk_strata = current_value - total_invested_symbol
            daily_states.append((tr, zysk_strata))
            print(f"Added ({buy_type}): amount={amount}, price={buy_price:.2f}, shares={shares_bought:.4f}, total_small_count={small_buy_total_count}, sequence_count={small_buy_sequence_count}")
            print(f"Value={current_value:.2f}, invested={total_invested_symbol:.2f}, zysk={zysk_strata:.2f}")
