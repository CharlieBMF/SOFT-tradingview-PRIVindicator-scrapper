...
for _, symbol_row in df_symbols.iterrows():
    ...
    for _, row in df_data.iterrows():
        ...
        if position_open:
            ...
            if len(valid_vals) >= 10:  # sprawdzamy dopiero gdy mamy pełne 10
            ...
            if pd.notna(ind_5):
                if zysk_strata >= 0:  # If at profit or break-even, use original conditions
                    if ind_5 < -7:
                        should_sell = True
                        sell_reason = "ind_5 < -7"
                ...
                else:  # If at a loss, only sell if ind_5 < -10
                    if ind_5 < -10:
                        should_sell = True
            ...
            if should_sell:
                ...
                ind_5_last_values.clear()  # reset po sprzedaży
                continue

        # Check for buy (open or add to position)
        if ind_22 > 3 or ind_7 > 0:
            # Determine amount by summing contributions from each condition
            amount = 0.0
            buy_conditions = []
            if ind_22 == 6:
                amount += 10.0
                buy_conditions.append("ind_22 == 6 ($10)")
            if ind_22 == 9:
                amount += 30.0
                buy_conditions.append("ind_22 == 9 ($30)")
            if ind_7 == 1:
                amount += 10.0
                buy_conditions.append("ind_7 == 1 ($10)")

            if amount == 0:  # No conditions met
                continue  # Skip buy
...
# New section: Summary of Profit/Loss for Open Positions
...
# List all open positions in the specified format
if open_positions:
...
if not df_invested.empty:
    max_invested_capital = df_invested.groupby('tr')['invested'].sum().max()
else:
