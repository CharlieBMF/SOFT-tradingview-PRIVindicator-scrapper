        ...
        if position_open:
            ...
            if len(valid_vals) >= 10:
                ...
                if below_zero_count >= 6:
                    if zysk_strata >= 0:  # jesteśmy na plusie lub break-even
                        should_sell = True
                        sell_reason = "ind_5 < 0 in at least 7 of last 10 TR"
                    else:
                        pending_sell = True
                        pending_reason = "ind_5 < 0 in at least 7 of last 10 TR (pending)"
            ...
            if pd.notna(ind_5):
                ...
                else:  # jesteśmy na stracie
                    if ind_5 < -10:
                        should_sell = True
                        sell_reason = "ind_5 < -10 (at a loss)"
                    elif ind_5 < -7 or ind_5_below_minus_5_count >= 3:
                        # sygnał jest, ale jesteśmy na stracie -> odkładamy sprzedaż
                        pending_sell = True
                        pending_reason = f"signal ({'ind_5 < -7' if ind_5 < -7 else 'ind_5 < -5 x3'}) pending"

            # Jeśli mamy sprzedaż odroczoną, to sprawdzamy czy wyszliśmy na 0+
            if pending_sell and zysk_strata >= 0:
                should_sell = True
                sell_reason = f"pending sell triggered: {pending_reason}"
                pending_sell = False
                pending_reason = ""
