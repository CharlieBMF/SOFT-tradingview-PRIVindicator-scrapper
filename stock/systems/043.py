...
for _, symbol_row in df_symbols.iterrows():
    ...
    for _, row in df_data.iterrows():
        ...
        if position_open:
            ...
            if sell_condition_triggered:
                ...
                if current_value <= max_value_after_trigger * 0.915:
                    ...
                    print(
                        f"Sold due to {sell_reason} and 3% drop from max value after trigger: zysk={zysk:.2f} ({length} days)")
                    position_open = False
                    total_shares = 0
                    total_invested_symbol = 0
                    num_purchases = 0
        ...
        if ind_22 > 3 or ind_7 > 0:
            ...
            if position_open:
                daily_states.append((tr, zysk_strata))
            print(
                f"{'Opened' if num_purchases == 1 else 'Dokup'}: amount={amount}, price={buy_price:.2f}, shares={shares_bought:.4f}")
    ...
    if position_open:
        ...
        print(
            f"Open position at end: value={current_value:.2f}, invested={total_invested_symbol:.2f}, zysk={zysk_strata:.2f}")
...
if open_positions:
    ...
    for p in open_positions:
        print(
            f"dla symbolu {p['symbol']} pozycja otwarta przez {p['length']} dni o łącznej wartości {p['max_value']:.2f}, aktualny zysk/strata pozycji {p['zysk']:.2f} usd czyli {p['percent_zysk']:.2f}%")
else:
    print("\n=== Lista Otwartych Pozycji ===")
    print("Brak otwartych pozycji.")
...
if top_10_positions:
    for p in top_10_positions:
        print(
            f"pozycja na symbolu o nazwie {p['symbol']} była otwarta przez {p['length']} dni od TickerRelative {p['open_tr']} do TickerRelative {p['close_tr']} z liczbą dokupień {p['num_purchases']}")
else:
...
print(
    f"Procentowy zysk (całkowity zrealizowany zysk / max zainwestowany kapitał): {percent_realized_zysk_max_invested:.2f}%")
print(
    f"Największy łączny koszt otwartych pozycji dla wszystkich symboli: {max_invested_capital:.2f} dla TickerRelative = {max_invested_tr if max_invested_tr is not None else 'N/A'}")

# New: Group by TickerRelative in 250-day increments
print("\n=== Analysis by TickerRelative Groups (250-day increments) ===")
if global_positions:
    # Find min and max TickerRelative
    min_tr = min(p['open_tr'] for p in global_positions)
    max_tr = max(p['close_tr'] for p in global_positions)

    # Create bins from min_tr to max_tr in steps of 250
    bins = range(int(min_tr), int(max_tr) + 250, 250)
    labels = [f"{start}-{start + 249}" for start in bins[:-1]]

    # Assign each position to a group based on close_tr
    df_positions = pd.DataFrame(global_positions)
    df_positions['tr_group'] = pd.cut(df_positions['close_tr'], bins=bins, labels=labels, include_lowest=True)

    # Calculate metrics for each group
    for group in labels:
        group_positions = df_positions[df_positions['tr_group'] == group]
        if group_positions.empty:
            continue
        closed_group_positions = group_positions[group_positions['status'].isna()]
        open_group_positions = group_positions[group_positions['status'] == 'open']

        print(f"\nGroup {group}:")

        # Metrics
        group_realized_zysk = sum(p['zysk'] for _, p in closed_group_positions.iterrows())
        group_unrealized_zysk = sum(p['zysk'] for _, p in open_group_positions.iterrows())
        group_total_zysk = group_realized_zysk + group_unrealized_zysk

        print(f"Sumaryczny zrealizowany zysk: {group_realized_zysk:.2f} $")
        print(f"Sumaryczny niezrealizowany zysk: {group_unrealized_zysk:.2f} $")
        print(f"Sumaryczny całkowity zysk: {group_total_zysk:.2f} $")

        if closed_group_positions.empty:
            print("Sredni czas otwarcia pozycji: 0.00 days")
            print("Najdluzszy czas otwarcia pozycji: 0 days")
            print("Najwieksza wartosc pozycji: 0.00 $")
            print("Liczba pozycji zamknietych na zysku: 0")
            print("Liczba pozycji zamknietych na stracie: 0")
        else:
            avg_length = closed_group_positions['length'].mean()
            max_length = closed_group_positions['length'].max()
            max_value_pos = closed_group_positions.loc[closed_group_positions['max_value'].idxmax()]
            profitable_closes = len(closed_group_positions[closed_group_positions['zysk'] > 0])
            loss_closes = len(closed_group_positions[closed_group_positions['zysk'] < 0])
            print(f"Sredni czas otwarcia pozycji: {avg_length:.2f} days")
            print(f"Najdluzszy czas otwarcia pozycji: {max_length} days (symbol: {max_value_pos['symbol']})")
            print(f"Najwieksza wartosc pozycji: {max_value_pos['max_value']:.2f} $ (symbol: {max_value_pos['symbol']})")
            print(f"Liczba pozycji zamknietych na zysku: {profitable_closes}")
        ...
        group_total_invested = group_positions['final_invested'].sum()
        group_invested_data = [d for d in global_invested_data if
                               d['tr'] <= group_positions['close_tr'].max() and d['tr'] >= group_positions[
                                   'open_tr'].min()]
        df_group_invested = pd.DataFrame(group_invested_data)
        group_max_invested = df_group_invested.groupby('tr')[
            'invested'].sum().max() if not df_group_invested.empty else 0

        if group_total_invested > 0:
            percent_zysk = (group_total_zysk / group_total_invested) * 100
        else:
            percent_zysk = 0
        print(f"Procentowy zysk (całkowity zysk / całkowity zainwestowany kapitał): {percent_zysk:.2f}%")

        if group_max_invested > 0:
            percent_zysk_max_invested = (group_total_zysk / group_max_invested) * 100
            percent_realized_zysk_max_invested = (group_realized_zysk / group_max_invested) * 100
        else:
            percent_zysk_max_invested = 0
            percent_realized_zysk_max_invested = 0
        print(f"Procentowy zysk (całkowity zysk / max zainwestowany kapitał): {percent_zysk_max_invested:.2f}%")
