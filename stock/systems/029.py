...
for _, symbol_row in df_symbols.iterrows():
    ...
    ind_5_last_values = deque(maxlen=5)
    ...
    for _, row in df_data.iterrows():
        ...
        if position_open:
            ...
            if len(valid_vals) >= 5:  # sprawdzamy dopiero gdy mamy pełne 5
                below_zero_count = sum(1 for v in valid_vals if v < -5)
                if below_zero_count >= 3 and zysk_strata >= 0:
                    should_sell = True
